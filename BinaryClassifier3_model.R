...
## Variable selection
null <- glm(outcomeName ~ 1, data=training, family=binomial)
full <- glm(outcomeName ~ ., data=training, family=binomial)
...
stepwise <- step(null, scope = list(upper=full), data=training, direction="both", trace=0)
summary(stepwise)
varImp(stepwise)
...
glmStep <- glm(outcomeName ~ ., data=training[,c(stepwiseVars,'outcomeName')], family=binomial)

#Plot ROC for training and AUC for training
library(ROCR)
p <- predict(glmStep, newdata=subset(training,select=stepwiseVars), type="response")
...
pred.glmStep <- as.vector(p)
(roc.glmStep <- pROC::roc(training$outcomeName2, pred.glmStep))
(auc.glmStep <- pROC::auc(roc.glmBoostModel))
...
for (i in names(which(sapply(training[,stepwiseVars], function(x) is.character(x))))) {
...
#######################
## GLM Boost Model
glmBoostModel <- train(outcomeName2 ~ ., data = trainingF, 
                       method = "glmboost", metric="ROC", 
                       trControl = fitControl, tuneLength=5, center=TRUE, 
                       family=Binomial(link = c("logit")))
pred.glmBoostModel <- as.vector(predict(glmBoostModel, newdata=trainingF, type="prob")[,"Spend"])
(roc.glmBoostModel <- pROC::roc(trainingF$outcomeName2, pred.glmBoostModel))
(auc.glmBoostModel <- pROC::auc(roc.glmBoostModel))

varImp(glmBoostModel)
glmBoostVars <- c('f1', 'f3', 'f35', 'f73', 'f97', 'f77')
...
glmModel <- train(outcomeName2 ~ ., data = trainingF[,c(glmBoostVars,'outcomeName2')], 
...
pred.glmModel <- as.vector(predict(glmModel, newdata=trainingF, type="prob")[,"Spend"])
...
(roc.glmModel <- pROC::roc(trainingF$outcomeName2, pred.glmModel))
...
fitControl_s <- trainControl(method = "repeatedcv",
                           ...
                           ## the following function
                           summaryFunction = twoClassSummary)
train_s <- createDataPartition(trainingF$outcomeName2, p=0.3, list=FALSE)
...
#######################
## GLMNET Model
glmnetModel <- train(outcomeName2 ~ ., data = trainingF_s[,c(rfVars,'outcomeName2')],
                     method='glmnet',  metric = "ROC", trControl=fitControl_s)
pred.glmnetModel <- as.vector(predict(glmnetModel, newdata=train_test, type="prob")[,"Spend"])
predCat <- ifelse(pred.glmnetModel>0.5,"Spend","Reduce")
(cm <- confusionMatrix(data=predCat, train_test$outcomeName2))
for (i in seq(0.1,0.9,0.1)) {
...
(roc.glmnetModel <- pROC::roc(train_test$outcomeName2, pred.glmnetModel))
